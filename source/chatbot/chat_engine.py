...
def _handle_semantic(question: str) -> str:
    ...
    try:
        ...
        if isinstance(retrieval_result, dict):
            if not retrieval_result.get("status", False):
                logger.warning("Retriever returned no usable context for question: %s", question)
                return retrieval_result.get("message") or FALLBACK_MESSAGE
